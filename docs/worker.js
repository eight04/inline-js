...
_module_exports_$3.makeDataUriToBuffer = void 0;
/**
...
_module_exports_$3.makeDataUriToBuffer = makeDataUriToBuffer;

const _module_exports_$2 = {};
Object.defineProperty(_module_exports_$2, "__esModule", { value: true });
_module_exports_$2.dataUriToBuffer = void 0;
 ...
 * @param {String} uri Data URI to turn into a Buffer instance
 */
_module_exports_$2.dataUriToBuffer = (0, _module_exports_$3.makeDataUriToBuffer)({ stringToBuffer, base64ToArrayBuffer });

let _module_exports_$1 = {};
//     treeify.js
...
(function (root, factory) {
  ...
  if (typeof _module_exports_$1 === 'object') {
    _module_exports_$1 = factory();
...
const _export_read_ = function (buffer, offset, isLE, mLen, nBytes) {
  var e, m;
...
const _export_write_ = function (buffer, value, offset, isLE, mLen, nBytes) {
  var e, m, c;
  var eLen = (nBytes * 8) - mLen - 1;
  var eMax = (1 << eLen) - 1;
  var eBias = eMax >> 1;
...
let _export_INSPECT_MAX_BYTES_;
const customInspectSymbol =
  (typeof Symbol === 'function' && typeof Symbol['for'] === 'function') // eslint-disable-line dot-notation
    ? Symbol['for']('nodejs.util.inspect.custom') // eslint-disable-line dot-notation
    : null;
_export_INSPECT_MAX_BYTES_ = 50;
...
Buffer.prototype.inspect = function inspect () {
  let str = '';
  const max = _export_INSPECT_MAX_BYTES_;
...
Buffer.prototype.readFloatLE = function readFloatLE (offset, noAssert) {
  offset = offset >>> 0;
  if (!noAssert) checkOffset(offset, 4, this.length);
  return _export_read_(this, offset, true, 23, 4)
};

Buffer.prototype.readFloatBE = function readFloatBE (offset, noAssert) {
  offset = offset >>> 0;
  if (!noAssert) checkOffset(offset, 4, this.length);
  return _export_read_(this, offset, false, 23, 4)
};

Buffer.prototype.readDoubleLE = function readDoubleLE (offset, noAssert) {
  offset = offset >>> 0;
  if (!noAssert) checkOffset(offset, 8, this.length);
  return _export_read_(this, offset, true, 52, 8)
};

Buffer.prototype.readDoubleBE = function readDoubleBE (offset, noAssert) {
  offset = offset >>> 0;
  if (!noAssert) checkOffset(offset, 8, this.length);
  return _export_read_(this, offset, false, 52, 8)
};
...
function writeFloat (buf, value, offset, littleEndian, noAssert) {
  ...
  _export_write_(buf, value, offset, littleEndian, 23, 4);
  return offset + 4
}
...
function writeDouble (buf, value, offset, littleEndian, noAssert) {
  ...
  _export_write_(buf, value, offset, littleEndian, 52, 8);
  return offset + 8
}
...
// (run `npm run build`).
_module_exports_.default = /((['"])(?:(?!\2|\\).|\\(?:\r\n|[\s\S]))*(\2)?|`(?:[^`\\$]|\\[\s\S]|\$(?!\{)|\$\{(?:[^{}]|\{[^}]*\}?)*\}?)*(`)?)|(\/\/.*)|(\/\*(?:[^*]|\*(?!\/))*(\*\/)?)|(\/(?!\*)(?:\[(?:(?![\]\\]).|\\.)*\]|(?![\/\]\\]).|\\.)+\/(?:(?!\s*(?:\b|[\u0080-\uFFFF$\\'"~({]|[+\-!](?!=)|\.?\d))|[gmiyus]{1,6}\b(?![\u0080-\uFFFF$\\]|\s*(?:[+\-*%&|^<>!=?({]|\/(?![\/*])))))|(0[xX][\da-fA-F]+|0[oO][0-7]+|0[bB][01]+|(?:\d*\.\d+|\d+\.?)(?:[eE][+-]?\d+)?)|((?!\d)(?:(?!\s)[$\w\u0080-\uFFFF]|\\u[\da-fA-F]{4}|\\u\{[\da-fA-F]+\})+)|(--|\+\+|&&|\|\||=>|\.{3}|(?:[+\-\/%&|^]|\*{1,2}|<{1,2}|>{1,3}|!=?|={1,2})=?|[?~.,:;[\](){}])|(\s+)|(^$|[\s\S])/g;

_module_exports_.matchToToken = function(match) {
...
const {default: jsTokens, matchToToken} = _module_exports_;
...
var _require__$binary_extensions_json_ = [
	"3dm",
	"3ds",
	"3g2",
	"3gp",
...
const extensions = new Set(_require__$binary_extensions_json_);

var isBinaryPath = filePath => extensions.has(posix.extname(filePath).slice(1).toLowerCase());

const fsReadFile = _export_promisify_(readFile$1);
...
function buildDependency(root, children) {
  return `${root}\n${_module_exports_$1.asTree(createTree(children))}`;
}
  ...
  try {
    files.forEach(file => {
      if (file.type === "text") {
        ...
        writeFileSync(file.name, _module_exports_$2(file.data), {flag: "wx"});
      }
    });
